...
class CAV(object):
  ...
  def default_hparams():
    ...
    """
    return tf.contrib.training.HParams(model_type='logistic', alpha=.01)
  ...
  def train(self, acts):
    ...
    tf.logging.info('using new version')
    self.accuracies = self._train_and_save_lm(lm, x, labels, labels2text)
    #self.accuracies = self._train_lm(lm, x, labels, labels2text)
  ...
  def _train_and_save_lm(self, lm, x, y, labels2text):
    """Train a model to get CAVs.
    ...
    save_path_lin_sep = self.save_path[:-4] + '_test.pkl'
    tf.logging.info(save_path_lin_sep)

    if save_path_lin_sep is not None:
      with tf.gfile.Open(save_path_lin_sep, 'w') as pkl_file:
       pickle.dump(save_dict, pkl_file)
