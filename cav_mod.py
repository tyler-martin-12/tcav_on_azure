...
class CAV(object):
  ...
  def train(self, acts):
    ...
    self.ls_dir = '/Users/tyler/Desktop/dissertation/programming/tcav/ls'
    #self._save_lin_sep()
  ...
  def _save_lin_sep(self):
    """Save a dictionary of this CAV to a pickle."""
    save_path_lin_sep = self.ls_dir
    #save_path_lin_sep = os.path.join(cav_dir,CAV.cav_key(concepts, bottleneck, cav_hparams.model_type,cav_hparams.alpha).replace('/', '.') + '_lin_sep.pkl')
    save_dict = {'x_train': self.x_train}

    if self.save_path_lin_sep is not None:
      with tf.gfile.Open(self.save_path_lin_sep, 'w') as pkl_file:
       pickle.dump(save_dict, pkl_file)
     tf.logging.info('did save save_path_lin_sep')
    else:
      tf.logging.info('save_path_lin_sep is None. Not saving anything')
  ###
